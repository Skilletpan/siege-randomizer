 ...
 * @param {Object[]} [options.exclude] Specific operators to exclude from the pick (banned, previous pick etc).
 * @param {String[]} [options.roles]   The roles the operator should have.
 * @param {String}   [options.side]    The side to pick the operator from.
...
export function pickRandomOperators(options = {}) {
  ...
  const pool = OPERATORS.filter((o) => {
    ...
    if (options.side && o.side !== options.side) return false;
    if (options.exclude && options.exclude.some((e) => e.name === o.name)) return false;
  ...
  return pickRandom(pool, options.amount);
}
